"""
Test suite for Wu-Palmer (WUP) similarity algorithm.
...
from algorithms.wup import build_wup_similarity, ensure_nltk_resources
...
def run_evaluations(project_root):
    """Run all evaluations for Wu-Palmer similarity."""
    ensure_nltk_resources()
    similarity = build_wup_similarity()
...
def pretty_print(evaluations):
    ...
    print("\n" + "="*80)
    print("WU-PALMER (WUP) SIMILARITY - EVALUATION RESULTS")
    print("="*80)
...
def main():
    """Main entry point for Wu-Palmer tests."""
    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
